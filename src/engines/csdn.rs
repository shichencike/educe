//! CSDN 搜索适配器（JS 渲染桥）。
//! 专用选择器：`.search-list-con .list-item` -> `a.title`（标题/链接），摘要 `.summary`。
...
pub const META: EngineMeta = EngineMeta {
    id: Cow::Borrowed("csdn"),
    name: Cow::Borrowed("CSDN"),
...
impl Csdn {
    fn parse_html(&self, html: &str, max: usize) -> Vec<SearchResult> {
        ...
        let Ok(result_sel) = Selector::parse(".search-list-con .list-item") else {
            return Vec::new();
        };
        let Ok(link_sel) = Selector::parse("a.title") else {
            return Vec::new();
        };
        let Ok(snip_sel) = Selector::parse(".summary") else {
            return Vec::new();
        };
        let mut out = Vec::new();
