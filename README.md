...
- 🔮 **搜索建议**：`/api/suggest` **并发**代理 DuckDuckGo/百度建议接口（各带 3s 硬超时，DDG 失败即时取百度），前端输入框实时下拉补全
...
- 📱 **SearXNG 风格前端**：搜索建议下拉、即时搜索防抖、键盘导航（↑↓ 选建议 / ←→ 翻页 / Enter 搜索 / Ctrl+K 聚焦）、分页、结果卡片增强（域名/关键词高亮/来源徽章）、搜索后 hero 紧凑态、回到顶部按钮、流式实时进度（已返回 X/Y 源 · 累计 N 条 · 已用 Xs）、响应式移动端，兼容安卓 8.1（Chrome 70 级 WebView）
- 🚀 **前后端性能优化**：关键词高亮单正则一次扫描、引擎状态行增量渲染、IndexedDB 连接复用、gzip 响应压缩（HTML/JSON 传输更小，SSE 自动豁免）、限速器热路径零分配、聚合容量预分配
...
搜索建议（自动补全），**并发**代理 DuckDuckGo / 百度建议接口（各自带 3s 硬超时，DDG 失败/为空时即时取百度结果，不串行等待）：
